        ...
        <div class="bg-gradient-to-r from-green-500 to-green-700 px-6 py-4">
          <div class="flex justify-between items-center">
            ...
            <a href="{{ route('dashboard.admin.users.create') }}"
               class="px-4 py-2 bg-white text-green-600 rounded-lg hover:bg-gray-100 font-medium flex items-center">
        ...
        <div class="px-6 py-3 bg-gray-50 border-b flex justify-between items-center">
          <div class="relative w-64">
            <input type="text" placeholder="Buscar usuarios..." 
                   class="w-full pl-10 pr-4 py-2 text-sm border rounded-lg focus:outline-none focus:ring-1 focus:ring-green-500">
          ...
          <div class="flex items-center space-x-4">
            <button class="text-sm text-gray-600 hover:text-green-600 flex items-center">
            ...
            <a href="{{ route('admin.users.export') }}" 
              class="text-sm text-gray-600 hover:text-green-600 flex items-center">
              <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <tr class="hover:bg-gray-50 transition-colors duration-150">
                  ...
                  <div class="flex items-center">
                    ...
                    @else
                      <div class="flex-shrink-0 h-10 w-10 bg-green-100 rounded-full flex items-center justify-center">
                        <span class="text-green-600 font-medium">{{ substr($user->nombre, 0, 1) }}</span>
                      </div>
                    @endif
                    <div class="ml-4">
                ...
                <td class="px-6 py-4 whitespace-nowrap">
                  <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full 
                    {{ $user->rol == 'admin' ? 'bg-green-100 text-green-800' : 
                       ($user->rol == 'nutriologo' ? 'bg-teal-100 text-teal-800' : 'bg-yellow-100 text-yellow-800') }}">
                    {{ ucfirst($user->rol) }}
                  </span>
                </td>
                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                  <div class="flex items-center justify-end space-x-4">
                    <a href="{{ route('dashboard.admin.users.show', $user->id) }}" 
                       class="text-gray-400 hover:text-green-600" title="Ver">
