    ...
    <main class="flex-1 p-8 overflow-y-auto">
      <div class="max-w-3xl mx-auto bg-white rounded-xl shadow-sm overflow-hidden">
        ...
        <div class="p-6 border-b border-gray-200 flex items-start">
          <div class="relative mr-6">
            @if(optional($user->perfil)->avatar)
              <img
                class="h-20 w-20 rounded-full object-cover border-2 border-green-200"
            ...
            @else
              <img
                class="h-20 w-20 rounded-full object-cover border-2 border-green-200"
                src="https://ui-avatars.com/api/?name={{ urlencode($user->nombre) }}&background=34D399&color=fff"
                alt="Foto de perfil"
              >
            @endif
          ...
          <div>
            ...
            <p class="text-sm text-gray-500 mt-1">
              Registrado: {{ \Carbon\Carbon::parse($user->creado_en)->format('d \d\e F Y') }}
            </p>
          </div>
        </div>
        ...
        <div class="bg-gray-50 px-6 py-4 flex justify-end">
          <a
            href="{{ route('dashboard.admin.profile') }}"
            class="px-5 py-2 border border-green-400 rounded-md text-sm font-medium text-green-600 bg-white hover:bg-green-50 transition"
          >
            Editar perfil
          </a>
        </div>
